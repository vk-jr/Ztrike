...
export function SignUp() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const userCredential = await signUp(formData.email, formData.password);      const { user } = userCredential;
      await createProfile(user, `${formData.firstName} ${formData.lastName}`);
      router.push('/profile');
    } catch (error: any) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    setError('');
    try {
      const result = await signInWithGoogle();      const { user } = result;
